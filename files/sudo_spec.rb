...
describe 'sudo::conf', type: :define do
  ...
  describe 'when creating a single sudo entry from epp template single_line_allocation.epp' do
    let(:title)    { 'dbas' }
    let(:filename) { '10_dbas' }
    let(:file_path) { '/etc/sudoers.d/10_dbas' }

    let :params do
      {
        priority:         10,
        template_epp:     { 'filename': 'sudo/single_line_allocation.epp',
                            'params': { 'user_spec': '%dbas', 'run_as': 'ALL', 'commands': 'ALL' } },
        sudo_config_dir:  '/etc/sudoers.d'
      }
    end

#    it do
#      is_expected.to contain_sudo__conf('dbas').with(template_epp: params[:template_epp])
#      is_expected.to contain_sudo__conf('dbas').with(priority: params[:priority],
#                                                    content: '%dbas ALL=(ALL) PASSWD: ALL')
#    end
#
#    it do
#      is_expected.to contain_file(filename).with(
